...
pub unsafe extern "C" fn wallet_build_and_sign_transaction(
    ...
    // Validate inputs
    if manager.is_null() || wallet.is_null() || outputs.is_null() || tx_bytes_out.is_null() || tx_len_out.is_null() {
    ...
    tx_builder = tx_builder.set_change_address(change_address).set_fee_level(FeeLevel::Normal);
